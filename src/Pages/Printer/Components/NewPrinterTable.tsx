...
import Coursing from "./NewPopUpPrinter";
...
const ComponentTable: React.FC = () => {
  ...
  const [selectedCoursing, setSelectedCoursing] = useState<IPrinter | null>(
    null
  );
  const [openDialog, setOpenDialog] = useState(false);
  const [openDeleteCategory, setOpenDeleteCategory] = useState(false);
  const [coursingToDelete, setCoursingToDelete] = useState<IPrinter | null>(
    null
  );

  const { data, isLoading, isError, refetch } = useGetPrinterQuery({
  ...
  const handleOpenDialog = (coursing: IPrinter) => {
  ...
  const handleOpenDeletePopup = (coursing: IPrinter) => {
  ...
  const [deleteCategory] = useDeletePrinterMutation();
  const handleDelete = async (id: string) => {
    try {
      ...
      if (response.status) {
        ...
        showErrorMessage("Failed to delete the printer");
      }
    } catch (error) {
      showErrorMessage("Failed to delete the printer");
    }
  ...
  const coursings: IPrinter[] = Array.isArray(data?.data)
    ? (data?.data as IPrinter[])
    : [];
    ...
    <Box>
      ...
      {coursingToDelete && (
        <DeletePopup
          ...
          content={`Are you sure you want to delete "${coursingToDelete.printerName}"?`}
        />
      )}
    </Box>
  );
