...
import { IProductBrand } from "../../../Api/Interface/api.interface";
import { appColor } from "../../../theme/appColor";
import Coursing from "./NewPopUpProductBrand";
...
const ComponentTable: React.FC = () => {
  ...
  const [selectedCoursing, setSelectedCoursing] =
    useState<IProductBrand | null>(null);
  ...
  const { data, isLoading, isError, refetch } = useGetProductBrandQuery({
  ...
  const handleOpenDialog = (coursing: IProductBrand) => {
  ...
  const handleOpenDeletePopup = (coursing: IProductBrand) => {
  ...
  const [deleteCategory] = useDeleteProductBrandMutation();
  const handleDelete = async (id: string) => {
    try {
      ...
      if (response.status) {
        ...
        showErrorMessage("Failed to delete the product brand");
      }
    } catch (error) {
      showErrorMessage("Failed to delete the product brand");
    }
  ...
  const coursings: IProductBrand[] = Array.isArray(data?.data)
    ? (data?.data as IProductBrand[])
    : [];
    ...
    <Box>
      ...
      {coursingToDelete && (
        <DeletePopup
          ...
          content={`Are you sure you want to delete "${coursingToDelete.productBrandName}"?`}
        />
      )}
    </Box>
  );
