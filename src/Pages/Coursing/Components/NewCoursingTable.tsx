...
import { ICoursing } from "../../../Api/Interface/api.interface";
...
const ComponentTable: React.FC = () => {
  ...
  const [selectedCoursing, setSelectedCoursing] = useState<ICoursing | null>(
    null
  );
  const [openDialog, setOpenDialog] = useState(false);
  const [openDeleteCategory, setOpenDeleteCategory] = useState(false);
  const [coursingToDelete, setCoursingToDelete] = useState<ICoursing | null>(
    null
  );

  const { data, isLoading, isError, refetch } = useGetCoursingQuery({
  ...
  const handleOpenDialog = (coursing: ICoursing) => {
  ...
  const handleOpenDeletePopup = (coursing: ICoursing) => {
  ...
  const [deleteCategory] = useDeleteCoursingMutation();
    ...
    <Box>
        ...
        >
          ...
          >
            <TextField
              ...
              placeholder="Search Coursing Name"
              sx={{ width: "100%" }}
              InputLabelProps={{
                sx: {
                  fontSize: 14,
      ...
      {coursingToDelete && (
        <DeletePopup
          ...
          content={`Are you sure you want to delete "${coursingToDelete.coursingName}"?`}
        />
      )}
    </Box>
  );
